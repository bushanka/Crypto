...
async def start():
    ...
    while True:
        ...
        #alya_list = [[383367365, -10], [489197056, -10]] * 25
        #start_time = time.time()
        to_send_info = []
        for iterate, user in enumerate(records_list):
            if is_user_sub[iterate][0] == 1:
                to_send_info.append({'id' : user[0], 'text': filter_params(user[2], text[0], text[1])}) # CHANGE TO USER[2] FOR NOT TEST
       # print("--- %s seconds to form the dict ---" % (time.time() - start_time))
        #print(len(to_send_info))
           # if is_user_sub[iterate][0] == 1:
                #filtered_text = filter_params(user[2], text[0], text[1])
        coros_1 = [send_info(user_id=usr['id'], msg=usr['text']) for usr in to_send_info]
        #start_time = time.time()
        await asyncio.gather(*coros_1)
        #print("--- %s seconds to send all msgs ---" % (time.time() - start_time))
